  ...
  : View{ graphics::Position{0,0}, size }
{
  ...
  m_menu = std::make_shared<graphics::VLayout>(menu_info, renderer, destination());
  m_play_text = std::make_shared<graphics::Text>(utils::Translations::translate(translations::MainMenu::Play), text_info, renderer );
  m_menu->addObject( m_play_text );
  m_settings_text = std::make_shared<graphics::Text>(utils::Translations::translate(translations::MainMenu::Settings), text_info, renderer );
  m_menu->addObject( m_settings_text );
  m_quit_text = std::make_shared<graphics::Text>(utils::Translations::translate(translations::MainMenu::Quit), text_info, renderer );
  ...
  m_logo = std::make_shared<graphics::Animation>( logo_info, renderer, destination() );
  addAnimation( m_logo );

  setFocus(m_play_text, renderer);
}

void MainMenu::setFocus(std::shared_ptr<graphics::Text>& to_select, SDL_Renderer* renderer)
{
  ...
  m_play_text->setColor( (to_select == m_play_text) ? m_selected_color : m_normal_color, renderer );
  m_settings_text->setColor( (to_select == m_settings_text) ? m_selected_color : m_normal_color, renderer );
  m_quit_text->setColor( (to_select == m_quit_text) ? m_selected_color : m_normal_color, renderer );
}
